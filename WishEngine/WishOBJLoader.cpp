...
namespace Wish
{
	...
	wish_mesh* WishOBJLoader_Load(const char* fileName) {
		...
		try{
			...
			std::vector<Vertex_VNT> meshVertices(faces.size() * 3);
			std::vector<GLuint> meshIndices(faces.size() * 3);
			...
			for (u32 i = 0; i < faces.size(); i++)
			{
				...
				meshVertices[i * 3] = (p1);
				meshVertices[i * 3 + 1] = (p2);
				meshVertices[i * 3 + 2] = (p3);

				meshIndices[i * 3] = (vertOff++);
				meshIndices[i * 3 + 1] = (vertOff++);
