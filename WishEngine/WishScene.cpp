...
namespace Wish
{
	...
	void wish_scene::Init()
	{	
		Root = Wish_Scene_NewTransform("Root");
		Wish_Memory_Zero(&Camera, sizeof(wish_camera));
		//Wish_Transform_Init(&scene.camera.Transform);
		Camera.Projection = mat4(1.0f);
		Camera.View = mat4(1.0f);
		Camera.ViewProjection = mat4(1.0f);
	}
	...
	void wish_scene::Process()
	{
		//Unpack scene
		Wish_Renderer_SetCamera(&Camera);
...
{
	wish_scene& scene = Wish_Engine_GetContext()->Scene;
	return scene.Root;
}
